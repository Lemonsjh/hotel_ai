...
class TestV26EdgePayloadContract(unittest.TestCase):
    def test_edges_reference_registered_nodes_and_have_payloads(self) -> None:
        nodes = {node["id"] for node in load_json(ROOT / "architecture" / "node_registry.json")["nodes"]}
        contract = load_json(ROOT / "contracts" / "v26" / "edge_payload_contract.yaml")
