...
def hotel_log_dir(hotel_id: str | None = None, purpose: str = "default") -> str:
    """返回按酒店隔离的日志目录。

    - 不传 hotel_id：返回全局 DEFAULT_LOG_DIR（向后兼容，现网璞悦未传 hotel_id 时走此路径）。
    - 传 hotel_id：返回 DEFAULT_LOG_DIR/<purpose>/<hotel_id>，实现多酒店日志隔离。
    优先级：HOTEL_OTA_LOG_DIR 显式配置 > 按 hotel 分目录 > 全局默认。
    """
    base = os.environ.get("HOTEL_OTA_LOG_DIR") or DEFAULT_LOG_DIR
    if not hotel_id:
        return base
    safe_id = "".join(ch for ch in str(hotel_id).strip().lower() if ch.isalnum() or ch in ("-", "_")) or "default"
    return str(Path(base) / purpose / safe_id)
