...
def approval_gate(
    *,
    ...
    if approver_role in {"admin", "owner"}:
        return {"allowed": True, "approval_required": False, "reason": "approved"}

    # S6 price execution ends at the price-task outbox. Operators are allowed
    # to confirm and enqueue only the exact S6 price preview they requested
    # themselves. approved_by/requester_id are canonical principal ids; the
    # executor_id may be a Feishu open_id/user_id, so do not compare those
    # different identifier schemes as raw strings.
    if approver_role == "operator" and action_type == "price_update":
        actor = str(approved_by or "").strip()
        requester = str(requester_id or "").strip()
        executor = str(executor_id or "").strip()
        if actor and actor == requester and executor:
            return {
                "allowed": True,
                "approval_required": False,
                "reason": "operator_self_confirmed_price_execution",
            }
        return {
            "allowed": False,
            "approval_required": True,
            "reason": "operator_price_execution_requires_same_requester",
